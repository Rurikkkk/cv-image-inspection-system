...
from image_markup.photo_alert.yolo_detector import detect_and_annotate
import os
import json
from config import config

markup_bp = Blueprint("markup", __name__)
...
def process_image():
    ...
    save_dir = config.MARKUPED_IMAGES_DIR
    os.makedirs(save_dir, exist_ok=True)
    markuped_filename = f"markuped_{filename}"
    markup_path = os.path.join(save_dir, markuped_filename)
